...
// tableau de facture
//
// $fLst est une array qui contient les infos à afficher
public function factureAdd($facture){
	array_push($this->factureLst, $facture);
}
// - - - - - - - - - - - - - - - - - - - - - - - - -
...
public function buildPDF(){
	...
	if(!empty($this->factureLst)){
		...
		$nb = 0;
		$fillRect = 1;
		foreach ($this->factureLst as $productLst ) {
		$this->prepareLine('', $this->template['totalHead'], 0);

		$this->buildLine(10, $this->template['client']['lineHeight'], 70, [$productLst->client], $this->template['client'], $this->template['total']['color'], $this->template['total']['backgroundColor']);	
		
			foreach($productLst->productLst as $r){
				$tplt = $this->template['product'];
				$nb++;
				$posMaxY = 0;
				$nbLineMax = 0;
				...
				$this->buildLine($this->productWidth, $cellHeightMax, $this->productHead, $r, $tplt, $fg, $bg);
					$this->Rect($this->GetX()+3, $this->GetY()-2-$cellHeightMax, 5, 5, 'D', array('all' => $style3));
			}
			if(!empty($productLst->total[1])){
				//$this->prepareLine('', $this->template['totalHead'], 0);
				$tplt = $this->template['total'];
					$this->buildLine($this->totalWidth, $tplt['lineHeight'], $this->totalHead, $productLst->total, $tplt, $tplt['color'], $tplt['backgroundColor']);//
			}
	}
	}
	// affiche le total
...
private function buildLine($aW, $aH, $aHeader='', $aContent='', $aTplt='', $aColorF=array('r'=>0, 'g'=>0, 'b'=>0), $aColorBk=array('r'=>255, 'g'=>25, 'b'=>255)){
